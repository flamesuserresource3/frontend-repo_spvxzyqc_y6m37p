...
export default function Process() {
  return (
    <section id="process" className="relative bg-gradient-to-b from-white to-indigo-50/40 py-20">
      <div className="mx-auto max-w-6xl px-6">
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-900 md:text-4xl">Process</h2>
          <p className="mt-3 text-gray-600">Clear milestones so you always know what’s next.</p>
        </div>

        <ol className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((s, idx) => (
            <li key={s.title} className="relative rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="absolute -top-3 -left-3 flex h-10 w-10 items-center justify-center rounded-full bg-indigo-600 text-white">
                {idx + 1}
              </div>
